    ...
    I: Iterator<Item = Event<'a>>,
{
    ...
    while let Some(event) = events.next() {
        ...
        match event {
            ...
            Event::Start(Tag::CodeBlock(kind)) => blocks.push(parse_code_block(events, kind)),
            Event::Rule => blocks.push(MarkdownBlock::Rule),
            Event::Start(Tag::Table(_)) => blocks.push(parse_table(events)),
...
fn parse_list<'a, I>(events: &mut Peekable<I>, start: Option<u64>) -> MarkdownBlock
where
    I: Iterator<Item = Event<'a>>,
{
...
fn parse_table<'a, I>(events: &mut Peekable<I>) -> MarkdownBlock
where
    I: Iterator<Item = Event<'a>>,
{
...
const fn heading_level(level: HeadingLevel) -> u32 {
    match level {
        HeadingLevel::H1 => 1,
        HeadingLevel::H2 => 2,
        HeadingLevel::H3 => 3,
