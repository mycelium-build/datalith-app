...
mod tests {
    ...
    use crate::ui::viewers::base::graph::model::{
        INITIAL_LAYOUT_RADIUS, INITIAL_LAYOUT_REFERENCE_NODES,
    };
    ...
    fn linked_nodes_converge_without_non_finite_motion() {
        let mut snapshot = super::super::test_snapshot(
            "views:\n  - type: graph\n    name: G",
            vec![
                ("one.md".into(), vec!["two.md".to_string()], Vec::new()),
        ...
        let rows = (0..node_count)
            .map(|index| {
                let links = if index == 0 {
                    ...
                    vec!["node-0.md".to_string()]
                };
                (PathBuf::from(format!("node-{index}.md")), links, Vec::new())
            })
            .collect();
        let mut snapshot =
            super::super::test_snapshot("views:\n  - type: graph\n    name: G", rows);
        let mut simulation = Simulation::default();
