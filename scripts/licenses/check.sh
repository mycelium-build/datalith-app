...
if ! git diff --exit-code --quiet -- THIRD-PARTY-NOTICES.md scripts/licenses/about.toml; then
    echo "LICENSE-E201 generated license material is stale: run scripts/licenses/generate.sh" >&2
    git diff -- THIRD-PARTY-NOTICES.md scripts/licenses/about.toml >&2
    exit 1
fi
